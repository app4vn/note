...
// --- THÊM MỚI: Hàm hiển thị Banner Việc Gấp ---
function displayGlobalUrgentTask() {
    ...
    if (mostUrgentTodo && urgentNoteDetails) {
        ...
        if (diffDays < 0) {
            deadlineStatusText = `(🔴 QUÁ HẠN ${Math.abs(diffDays)} ngày)`;
            deadlineStatusClass = 'overdue';
        } else if (diffDays === 0) {
            deadlineStatusText = `(🔵 HÔM NAY)`;
            deadlineStatusClass = 'today';
        } else if (diffDays === 1) {
            deadlineStatusText = `(🟡 NGÀY MAI)`;
            deadlineStatusClass = 'tomorrow';
        } else {
            deadlineStatusText = `(Còn ${diffDays} ngày)`;
        }
        ...
        if (noteLink) {
            noteLink.addEventListener('click', () => {
                if (notesCache[urgentNoteDetails.id]) {
                    showDetailView(notesCache[urgentNoteDetails.id]);
                }
            });
        }
