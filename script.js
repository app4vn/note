...
function initializeCalendar() {
    ...
    calendar = new FullCalendar.Calendar(calendarContainer, {
        ...
        // --- THÊM MỚI: eventDidMount cho Tippy.js ---
        eventDidMount: function(info) {
            if (info.event.extendedProps) {
                let tooltipContent = `<strong>${info.event.title.replace(/^[📌✅⏳🔥🟢❌\s]+/, '')}</strong>`; // Loại bỏ icon prefix khỏi tiêu đề tooltip
                ...
                if (info.event.extendedProps.type === 'todo') {
                    ...
                    if (info.event.extendedProps.deadline) {
                        try {
                            tooltipContent += `<br>Hạn: ${new Date(info.event.extendedProps.deadline + "T00:00:00").toLocaleDateString('vi-VN')}`;
                        } catch(e){}
                    }
                    ...
                    // Có thể thêm nội dung ghi chú ngắn nếu muốn
                ...
                tippy(info.el, {
                    ...
                    allowHTML: true,
                    theme: 'light-border', // Sử dụng theme đã import
                    placement: 'top',
                    arrow: true,
